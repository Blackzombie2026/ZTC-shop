        ...
        {list.map(o=>(
          <Link key={o.id} to={`/orders/${o.id}`} className="block p-4 rounded-2xl bg-white/5 border border-white/10 hover:border-violet-500/30">
            <div className="flex flex-wrap justify-between gap-2">
              ...
              <span className="text-xs px-2 py-1 rounded-full font-bold bg-white/10 border border-white/10">{o.status}</span>
            </div>
            <div className="text-sm text-white/60 mt-1">{o.items.length} article(s) • {o.total.toFixed(2)} TND</div>
          </Link>
        ))}
      </div>
    </div>
